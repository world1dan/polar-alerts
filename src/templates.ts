...
> {
  return {
    ["checkout.created"]: async ({ data: checkout }: { data: Checkout }) => {
    ...
    ["checkout.updated"]: async ({ data: checkout }: { data: Checkout }) => {
      const description = new AlertDescriptionBuilder({
        config,
        escapeMarkdown,
      })
      ...
      return {
        ...
        description: await description
          ...
          .hashtags([
            "checkout",
            checkout.status === "succeeded" ? "succeeded" : "updated",
          ])
          .build(),
        silent: true,
      };
    ...
    ["subscription.created"]: async ({
      ...
      const description = new AlertDescriptionBuilder({
        ...
        .separator()
        .discountInfo(subscription.discount, undefined, subscription.currency)
        .moneyField(
          "💵 Amount",
          subscription.amount,
          true,
      ...
      description
        ...
        .separator()
        .link("View Subscription", getSubscriptionLink(config, subscription.id))
        .separator()
    ...
    ["subscription.canceled"]: async ({
      ...
      description.separator().dateField("Started on", subscription.startedAt);
    ...
    ["order.created"]: async ({ data: order }: { data: Order }) => {
      ...
      description
        ...
        .discountInfo(order.discount, order.discountAmount, order.currency)
        .moneyField(
          "🏛️ Tax",
          order.taxAmount ?? 0,
      ...
      if (order.metadata && Object.keys(order.metadata).length > 0) {
        description
          .separator()
          .field("Metadata", JSON.stringify(order.metadata, null, 2), "code");
      }
    ...
    ["customer.created"]: async ({ data: customer }: { data: Customer }) => {
      const description = new AlertDescriptionBuilder({
        config,
        escapeMarkdown,
      });
      ...
      if (customer.externalId) {
        description.separator().field("External ID", customer.externalId);
      }
    ...
    ["customer.updated"]: async ({ data: customer }: { data: Customer }) => {
      const description = new AlertDescriptionBuilder({
        config,
        escapeMarkdown,
      });
      ...
      if (customer.modifiedAt) {
        description.separator().dateField("Updated at", customer.modifiedAt);
      }
    ...
    ["customer.deleted"]: async ({ data: customer }: { data: Customer }) => {
      const description = new AlertDescriptionBuilder({
        config,
        escapeMarkdown,
      });
